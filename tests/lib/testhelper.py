...
class TestHelper:
    ...
    @db_session
    def create_importfile_list(self):
    ...
    def create_summary(self,no_dates=False):
        ...
        summary = { 
            ...
            'i_date': example_date,
            'f_date': example_date,
            'event_list': self.create_event_list(),
            'importfile_list': self.create_importfile_list(),
    ...
    def get_bad_content(self):
        with open(os.path.join(DATA_DIR, 'bad_content.pdf'), 'rb') as f:
            content = f.read()
            return content

    def put_file_in_bucket(self, file_name):
        data = TestHelper.get_content(self)
        ret = self.minio_manager.put_file(self.minio_manager.default_bucket, file_name, data)
