...
class TestDbUtils(TestCase):
    ...
    def test_listEvents(self):
        file1 = db.Event.get(key="proves/fit1.zip")
        file2 = db.Event.get(key="proves/fit2.zip")
        file3 = db.Event.get(key="proves/fit3.zip")

        self.assertEqual(file1.value['Records'][0]['s3']['object']['key'], "proves/fit1.zip")
        self.assertEqual(file1.value['Records'][0]['s3']['object']['eTag'], "87b416894d0d12580610f3e26a1120b8")

        self.assertEqual(file2.value['Records'][0]['s3']['object']['key'], "proves/fit2.zip")
        self.assertEqual(file2.value['Records'][0]['s3']['object']['eTag'], "59165058c94c753c75c492774d2c3db2")

        self.assertEqual(file3.value['Records'][0]['s3']['object']['key'], "proves/fit3.zip")
    ...
    def populate_impFile_database():
        imFile1 = ImportFile(etag="87b416894d0d12580610f3e26a1120b8", name="proves/fit1.zip", bucket="proves", size="5804", msg="")
        imFile2 = ImportFile(etag="59165058c94c753c75c492774d2c3db2", name="proves/fit2.zip", bucket="proves", size="4137", msg="")
        imFile3 = ImportFile(etag="a75b9c253823c292d69651890c73618a", name="proves/fit3.zip", bucket="proves", size="4691", msg="")
